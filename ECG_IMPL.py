...
if uploaded_file is not None:
    ...
    lowcut = st.sidebar.slider("Low Cutoff Frequency (Hz)", 0.1, 5.0, 0.5, 0.1)
    highcut = st.sidebar.slider("High Cutoff Frequency (Hz)", 10, 100, 50, 5)

    # Wavelet Transform Parameters
    wavelet_level = st.sidebar.slider("Wavelet Decomposition Level", 1, 5, 3, 1)
    threshold_factor = st.sidebar.slider("Wavelet Threshold Factor", 0.1, 1.0, 0.6, 0.1)

    # Outlier Removal (IQR)
    iqr_multiplier = st.sidebar.slider("IQR Multiplier (Outlier Removal)", 1.0, 3.0, 1.5, 0.1)

    # R-Peak Detection Parameters
    min_height = st.sidebar.slider("Minimum Peak Height", 0.1, 1.0, 0.5, 0.1)
    peak_distance = st.sidebar.slider("Minimum R-Peak Distance (ms)", 100, 500, 200, 10)
    ...
    peaks, _ = find_peaks(df_ecg["ECG Signal"], distance=peak_distance, height=min_height)
    rr_intervals = np.diff(df_ecg["Time (ms)"].iloc[peaks])
    ...
    ax.set_title("ECG Signal with R-Peaks (After Outlier Removal)")
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)

    # **🔹 Download Processed Data & Features**
